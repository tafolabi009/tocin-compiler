...
namespace parser {
    ...
    ast::StmtPtr Parser::declaration() {
        try {
            ...
            errorHandler.reportError(e.what(), std::string(previous().filename), previous().line,
                previous().column, error::ErrorSeverity::ERROR);
            synchronize();
            return nullptr;
        }
    }
    ...
    ast::StmtPtr Parser::statement() {
        if (match(lexer::TokenType::IF)) return ifStmt();
        if (match(lexer::TokenType::WHILE)) return whileStmt();
        if (match(lexer::TokenType::FOR)) return forStmt();
        if (match(lexer::TokenType::LEFT_BRACE)) return blockStmt();
        if (match(lexer::TokenType::RETURN)) return returnStmt();
        if (match(lexer::TokenType::MATCH)) return matchStmt();
    ...
    ast::ExprPtr Parser::assignment() {
        ...
        if (match(lexer::TokenType::EQUAL)) {
            ...
            if (auto var = std::dynamic_pointer_cast<ast::VariableExpr>(expr)) {
                return std::make_shared<ast::AssignExpr>(expr->token, var->name, value);
            }
            else if (auto get = std::dynamic_pointer_cast<ast::GetExpr>(expr)) {
                return std::make_shared<ast::SetExpr>(expr->token, get->object, get->name, value);
            }
    ...
    ast::ExprPtr Parser::primary() {
        ...
        if (match(lexer::TokenType::INT) || match(lexer::TokenType::FLOAT64) ||
            ...
            auto type = previous().type == lexer::TokenType::INT ? ast::LiteralExpr::LiteralType::INTEGER :
    ...
    void Parser::synchronize() {
        advance();
        while (!isAtEnd()) {
            if (previous().type == lexer::TokenType::SEMI_COLON) return;
            switch (peek().type) {
    ...
    void Parser::error(const lexer::Token& token, const std::string& message) {
        errorHandler.reportError(message, std::string(token.filename), token.line, token.column,
