...
private:
    ...
    {
        // Convert our optimization level to LLVM's representation
        ...
        // Create a FunctionAnalysisManager
        llvm::FunctionAnalysisManager FAM;
        
        // Create a ModuleAnalysisManager
        ...
        // Simple approach: just add standard scalar optimizations
        ...
        if (optLevel != llvm::OptimizationLevel::O0) {
            // Add some basic optimizations for O1 and above
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createCFGSimplificationPass()));
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createSROAPass()));
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createEarlyCSEPass()));
        }
        
        if (optLevel == llvm::OptimizationLevel::O2 || optLevel == llvm::OptimizationLevel::O3) {
            // Add more aggressive optimizations for O2 and O3
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createReassociatePass()));
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createLICMPass()));
        }
        
        if (optLevel == llvm::OptimizationLevel::O3) {
            // Add the most aggressive optimizations for O3 only
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createAggressiveDCEPass()));
            MPM.addPass(llvm::createModuleToFunctionPassAdaptor(llvm::createInstructionCombiningPass()));
        }
