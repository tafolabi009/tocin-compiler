...
namespace ast {
    ...
    using StmtPtr = std::shared_ptr<class Statement>;
    ...
    public:
        explicit Type(const lexer::Token& token) : Node(token) {}
    ...
    public:
        ...
        std::string toString() const override {
            ...
            for (size_t i = 0; i < typeArguments.size(); ++i) {
                ...
                if (i < typeArguments.size() - 1) result += ", ";
            }
            return result + ">";
        }
        std::string name;
    ...
    public:
        enum class LiteralType { INTEGER, FLOAT, BOOLEAN, STRING, NIL };
    ...
    public:
        AssignExpr(const lexer::Token& token, const std::string& name, ExprPtr value)
            : Expression(token), name(name), value(std::move(value)) {}
        void accept(Visitor& visitor) override;
        std::string name;
        ExprPtr value;
    ...
    /**
     * @brief Function declaration statement (e.g., def foo(x: int) -> int { ... }).
     */
    ...
    /**
     * @brief Class declaration statement (e.g., class C { ... }).
     */
    class ClassStmt : public Statement {
    public:
        ClassStmt(const lexer::Token& token, const std::string& name, std::vector<StmtPtr> fields, std::vector<StmtPtr> methods)
            : Statement(token), name(name), fields(std::move(fields)), methods(std::move(methods)) {}
        void accept(Visitor& visitor) override;
        std::string name;
    ...
    /**
     * @brief Import statement (e.g., import mod).
     */
    class ImportStmt : public Statement {
    public:
        ImportStmt(const lexer::Token& token, const std::string& module)
            : Statement(token), module(module) {}
        void accept(Visitor& visitor) override;
        std::string module;
    };
    ...
    public:
        NewExpr(const lexer::Token& token, TypePtr type) 
            : Expression(token), type(std::move(type)) {}
    ...
    public:
        DeleteExpr(const lexer::Token& token, Expression* expression) 
            : Expression(token), expression(expression) {}
        
        void accept(Visitor& visitor) override;

        TypePtr getType() const override {
            return expression ? expression->getType() : nullptr;
        }
