...
namespace runtime
{
    ...
    /**
     * A promise represents the producer side of an asynchronous result
     */
    template <typename T>
    class Promise {
    public:
        ...
        void setSuccess(T value) {
            std::lock_guard<std::mutex> lock(future.state->mutex);
            
            if (future.state->hasValue || future.state->hasError) {
                throw std::runtime_error("Promise already completed");
            }
            
            future.state->value = std::move(value);
            future.state->hasValue = true;
            future.state->runContinuations();
            future.state->condition.notify_all();
        }
        ...
        void setError(std::exception_ptr error) {
            ...
            future.state->error = error;
            future.state->hasError = true;
            future.state->runContinuations();
            future.state->condition.notify_all();
        }
        ...
        void setException(const E& exception) {
            setError(std::make_exception_ptr(exception));
        }
    ...
    public:
        ...
        void setSuccess() {
            ...
            if (future.state->isCompleted || future.state->hasError) {
                throw std::runtime_error("Promise already completed");
            }
            
            future.state->isCompleted = true;
            future.state->runContinuations();
            future.state->condition.notify_all();
        }
    ...
    auto whenAll(Futures&&... futures) {
        ...
        std::apply([&](auto&&... args) {
            ((processFuture(*resultsTuple, args, std::get<args>(std::forward_as_tuple(futures...)))), ...);
    ...
    auto whenAny(Futures&&... futures) {
        ...
        std::apply([&](auto&&... args) {
            ((processFuture(args, std::get<args>(std::forward_as_tuple(futures...)))), ...);
        }, std::make_index_sequence<sizeof...(Futures)>{});
