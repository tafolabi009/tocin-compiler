...
namespace lexer {
    ...
    void Lexer::scanNumber() {
        bool isFloat = false;
        bool isMultiLine = current + 2 < source.length() &&
            source[current] == '"' && source[current + 1] == '"' && source[current + 2] == '"';
        int startColumn = column;

        if (isMultiLine) {
            current += 3;
            column += 3;
            while (!isAtEnd() && !(current + 2 < source.length() &&
                source[current] == '"' && source[current + 1] == '"' && source[current + 2] == '"')) {
                if (source[current] == '\n') {
                    ++line;
                    column = 1;
                    ...
                    ++column;
                    if (isAtEnd()) {
                        errorHandler.reportError("Unterminated escape in multi-line string", filename, line,
                            column, error::ErrorSeverity::ERROR);
                        tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                        errorHandler.setFatal(true);
                        return;
                    }
                    ...
                    case 'u': {
                        if (current + 4 >= source.length()) {
                            errorHandler.reportError("Incomplete Unicode escape", filename, line, column,
                                error::ErrorSeverity::ERROR);
                            tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                            errorHandler.setFatal(true);
                            return;
                        }
                        std::string hex = source.substr(current + 1, 4);
                        try {
                            value += static_cast<char>(std::stoi(hex, nullptr, 16));
                        }
                        catch (...) {
                            errorHandler.reportError("Invalid Unicode escape: \\u" + hex, filename, line, column,
                                error::ErrorSeverity::ERROR);
                        }
                        current += 4;
                        column += 4;
                    ...
                    default:
                        ...
                        errorHandler.reportError("Invalid escape: \\" + std::string(1, source[current]),
                            filename, line, column, error::ErrorSeverity::WARNING);
                    }
                    ++current;
                    ++column;
                }
            ...
            if (isAtEnd()) {
                errorHandler.reportError("Unterminated multi-line string", filename, line, column,
                    error::ErrorSeverity::ERROR);
                tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                errorHandler.setFatal(true);
                return;
            ...
            char quoteType = source[current];
            ++current;
            ++column;
            while (!isAtEnd() && source[current] != quoteType) {
                if (source[current] == '\n') {
                    errorHandler.reportError("Newline in single-line string", filename, line, column,
                        error::ErrorSeverity::ERROR);
                    tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                    errorHandler.setFatal(true);
                    ++line;
                    ...
                    ++column;
                    if (isAtEnd()) {
                        errorHandler.reportError("Unterminated string escape", filename, line, column,
                            error::ErrorSeverity::ERROR);
                        tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                        errorHandler.setFatal(true);
                        return;
            ...
            if (isAtEnd()) {
                errorHandler.reportError("Unterminated string", filename, line, column,
                    error::ErrorSeverity::ERROR);
                tokens.emplace_back(TokenType::ERROR, value, filename, line, startColumn);
                errorHandler.setFatal(true);
                return;
        ...
        tokens.emplace_back(TokenType::STRING, value, filename, line, startColumn);
    }
    ...
    void Lexer::scanNumber() {
        ...
        tokens.emplace_back(isFloat ? (isFloat32 ? TokenType::FLOAT32 : TokenType::FLOAT64) : TokenType::INT,
            value, filename, line, column - value.length());
    }
    ...
    void Lexer::scanToken() {
        ...
        switch (c) {
        case '(': tokens.emplace_back(TokenType::LEFT_PAREN, "(", filename, line, column - 1); break;
        case ')': tokens.emplace_back(TokenType::RIGHT_PAREN, ")", filename, line, column - 1); break;
        case '{': tokens.emplace_back(TokenType::LEFT_BRACE, "{", filename, line, column - 1); break;
        case '}': tokens.emplace_back(TokenType::RIGHT_BRACE, "}", filename, line, column - 1); break;
        case '[': tokens.emplace_back(TokenType::LEFT_BRACKET, "[", filename, line, column - 1); break;
        case ']': tokens.emplace_back(TokenType::RIGHT_BRACKET, "]", filename, line, column - 1); break;
        case ',': tokens.emplace_back(TokenType::COMMA, ",", filename, line, column - 1); break;
        case '.': tokens.emplace_back(TokenType::DOT, ".", filename, line, column - 1); break;
        case ';': tokens.emplace_back(TokenType::SEMI_COLON, ";", filename, line, column - 1); break;
        case ':':
            if (match(':')) {
