...
namespace type_checker
{
    ...
    void TypeChecker::popScope()
    {
        if (environment_ && environment_ != globalEnv_) {
            ...
            auto parent = environment_->lookup("__parent");
            if (parent) {
                // Find the actual parent environment
                for (const auto& [name, val] : globalEnv_->variables_) {
                    if (val.first == parent) {
                        environment_ = globalEnv_;
                        return;
                    }
                }
                // Just go back to the global environment as fallback
                environment_ = globalEnv_;
            } else {
                environment_ = globalEnv_;
            }
