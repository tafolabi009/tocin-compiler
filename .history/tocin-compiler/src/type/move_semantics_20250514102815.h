...
namespace type_checker
{
    ...
    public:
        ...
        {
            if (parameter.getType()->isMoveable())
            {
                parameter.setMoved(true);
                return true;
            }
            return false;
        }
                                                   ...
                                                   const std::vector<ast::VariableStmt *> &fields)
        {
            ...
            for (ast::VariableStmt *field : fields)
            {
                ...
                auto fieldExpr = std::make_shared<ast::VariableExpr>(token, field->getName());
                auto otherFieldExpr = std::make_shared<ast::GetExpr>(token,
                                                                     std::make_shared<ast::VariableExpr>(token, "other"), field->getName());
            ...
            return new ast::FunctionStmt(token, className, params,
                                         std::make_shared<ast::ClassType>(token, className), body);
        }
