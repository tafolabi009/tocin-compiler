...
namespace type_checker
{
     ...
     */
    class TraitStmt : public ast::Statement
    {
    public:
        TraitStmt(const lexer::Token &token, const std::string &name,
            ...
            : Statement(token), name(name), typeParams(std::move(typeParams)),
              methods(std::move(methods)) {}

        virtual void accept(ast::Visitor &visitor) override
        {
            visitor.visitClassStmt(static_cast<ast::ClassStmt *>(this));
        }

        const std::string &getName() const { return name; }
        const std::vector<ast::TypeParameter> &getTypeParams() const { return typeParams; }
     ...
     */
    class ImplStmt : public ast::Statement
    {
    public:
        ImplStmt(const lexer::Token &token, ast::TypePtr type,
                 std::vector<ast::TypePtr> traits,
                 std::vector<ast::FunctionStmt *> methods)
            : Statement(token), type(type), traits(std::move(traits)),
              methods(std::move(methods)) {}
        ...
        const std::vector<ast::TypePtr> &getTraits() const { return traits; }
        const std::vector<ast::FunctionStmt *> &getMethods() const { return methods; }
    ...
    {
    public:
        TraitBound(const std::string &typeParameter,
    ...
    public:
        ...
        ast::FunctionStmt *findTraitMethod(ast::TypePtr type, ast::TypePtr traitType, const std::string &methodName)
        {
            ...
            if (traitIt != traits.end())
            {
                ...
                {
                    if (method->getName() == methodName && method->getBody() != nullptr)
                    {
                        return method;
                    }
                }
    ...
    private:
        ...
        bool verifyImplementation(TraitStmt *trait, ImplStmt *impl)
        {
            ...
            for (ast::FunctionStmt *requiredMethod : trait->getMethods())
            {
                ...
                if (requiredMethod->getBody() != nullptr)
                {
                    continue;
                }
                ...
                {
                    if (implMethod->getName() == requiredMethod->getName())
                    {
                        ...
                        if (!verifyMethodSignature(requiredMethod, implMethod))
                        {
                            errorHandler.reportError(
                                ...
                                "Method '" + requiredMethod->getName() + "' implementation doesn't match trait signature",
                                "", 0, 0, error::ErrorSeverity::ERROR);
                            return false;
                        }
                ...
                if (!found)
                {
                    errorHandler.reportError(
                        error::ErrorCode::T003_UNDEFINED_FUNCTION,
                        "Missing implementation for required method '" + requiredMethod->getName() + "'",
                        "", 0, 0, error::ErrorSeverity::ERROR);
                    return false;
                }
            }
        ...
        {
            // Check return type
            if (traitMethod->getReturnType()->toString() != implMethod->getReturnType()->toString())
            {
                return false;
            }

            // Check parameter count and types
            if (traitMethod->getParameters().size() != implMethod->getParameters().size())
            {
                return false;
            }

            for (size_t i = 0; i < traitMethod->getParameters().size(); ++i)
            {
                if (traitMethod->getParameters()[i].getType()->toString() !=
                    implMethod->getParameters()[i].getType()->toString())
                {
                    return false;
                }
            }
