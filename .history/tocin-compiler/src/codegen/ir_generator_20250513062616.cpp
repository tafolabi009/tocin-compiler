...
{
    // Use getTypeExpr() instead of typeExpr directly
    if (auto *typeExpr = expr->getTypeExpr().get())
    {
        if (auto *idExpr = dynamic_cast<ast::VariableExpr*>(typeExpr))
        {
            std::string typeName = idExpr->name.lexeme;
...
{
    // Use value instead of getExpression()
    stmt->value->accept(*this);
    if (!lastValue)
        return;
