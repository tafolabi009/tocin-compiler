...
namespace ir_generator
{
    ...
    void IRGenerator::visitBinaryExpr(ast::BinaryExpr *expr)
    {
        ...
        case lexer::TokenType::PLUS:
            ...
            {
                // Pointer arithmetic
                llvm::Type *pointeeType = llvm::cast<llvm::PointerType>(left->getType())->getElementType();
    ...
    {
        errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                 "Logical expression not implemented",
                                 "", 0, 0, error::ErrorSeverity::ERROR);
    ...
    {
        errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                 "'this' expression not implemented",
                                 "", 0, 0, error::ErrorSeverity::ERROR);
        lastValue = nullptr;
    }
    ...
    {
        errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                 "'super' expression not implemented",
                                 "", 0, 0, error::ErrorSeverity::ERROR);
        lastValue = nullptr;
    }
    ...
    {
        errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                 "Index expression not implemented",
                                 "", 0, 0, error::ErrorSeverity::ERROR);
        lastValue = nullptr;
    }
    ...
    {
        // Implement Option type codegen
        if (expr->isSome())
        {
            // First, visit the contained value
            expr->getValue()->accept(*this);
            llvm::Value *value = lastValue;
            if (!value)
                return;

            // Create an Option<T> struct instance
            // Determine the type
            ast::TypePtr valueType = typeChecker.getType(expr->getValue());
            ast::TypePtr optionType = std::make_shared<ast::OptionType>(valueType);
            llvm::Type *llvmOptionType = getLLVMType(optionType);

            if (!llvmOptionType)
                return;

            // Allocate space for the Option<T>
            llvm::AllocaInst *optionAlloca = builder.CreateAlloca(llvmOptionType, nullptr, "option");

            // Set hasValue field to true
            llvm::Value *hasValuePtr = builder.CreateStructGEP(llvmOptionType, optionAlloca, 0, "hasValue.ptr");
            builder.CreateStore(llvm::ConstantInt::get(llvm::Type::getInt1Ty(context), 1), hasValuePtr);

            // Set value field to the value
            llvm::Value *valuePtr = builder.CreateStructGEP(llvmOptionType, optionAlloca, 1, "value.ptr");
            builder.CreateStore(value, valuePtr);

            // Load the Option<T> struct to return it
            lastValue = builder.CreateLoad(llvmOptionType, optionAlloca, "option");
        }
        else // None case
        {
            // Determine the type - this is tricky for None as it doesn't have a value
            // For now, we'll handle it as Option<void> and report a warning
            errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                     "None literal not fully implemented - type inference needed",
                                     "", 0, 0, error::ErrorSeverity::WARNING);

            // TODO: Properly infer the type of None from context
            ast::TypePtr voidType = std::make_shared<ast::SimpleType>("void");
            ast::TypePtr optionType = std::make_shared<ast::OptionType>(voidType);
            llvm::Type *llvmOptionType = getLLVMType(optionType);
            ...
            // Set hasValue field to false
            llvm::Value *hasValuePtr = builder.CreateStructGEP(llvmOptionType, optionAlloca, 0, "hasValue.ptr");
            builder.CreateStore(llvm::ConstantInt::get(llvm::Type::getInt1Ty(context), 0), hasValuePtr);

            // Value field doesn't matter for None, but initialize it to null/zero
            llvm::Value *valuePtr = builder.CreateStructGEP(llvmOptionType, optionAlloca, 1, "value.ptr");
            builder.CreateStore(llvm::Constant::getNullValue(
                                    llvm::cast<llvm::StructType>(llvmOptionType)->getElementType(1)),
                                valuePtr);
    ...
    {
        // Similar implementation to OptionExpr
        if (expr->isOk())
        {
            // First, visit the contained value
            expr->getValue()->accept(*this);
            ...
            // Create a Result<T,E> struct instance
            // This is just a placeholder - type inference is needed for proper implementation
            errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                     "Ok expression not fully implemented - type inference needed",
                                     "", 0, 0, error::ErrorSeverity::WARNING);

            // TODO: Properly infer the types from context
            lastValue = nullptr;
        }
        else // Err case
        {
            // First, visit the contained error value
            expr->getValue()->accept(*this);
            llvm::Value *errValue = lastValue;
            if (!errValue)
                return;
            ...
            // This is just a placeholder - type inference is needed for proper implementation
            errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                     "Err expression not fully implemented - type inference needed",
                                     "", 0, 0, error::ErrorSeverity::WARNING);
