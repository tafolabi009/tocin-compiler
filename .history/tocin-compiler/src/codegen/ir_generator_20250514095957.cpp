...
void IRGenerator::visitDictionaryExpr(ast::DictionaryExpr *expr)
{
    ...
    // Allocate memory for keys array - use correct CreateMalloc signature
    llvm::Value *keysPtr = builder.CreateMalloc(
        llvm::Type::getInt8Ty(context), // Size type
        arraySize,                       // Number of elements
        keyType,                         // Type of each element
        nullptr,                         // Size mul operand
        nullptr,                         // Array size
        "dict.keys");
    
    // Allocate memory for values array - use correct CreateMalloc signature
    llvm::Value *valuesPtr = builder.CreateMalloc(
        llvm::Type::getInt8Ty(context), // Size type
        arraySize,                       // Number of elements
        valueType,                       // Type of each element
        nullptr,                         // Size mul operand
        nullptr,                         // Array size
        "dict.values");
...
void IRGenerator::generateMethod(const std::string &className, llvm::StructType *classType, ast::FunctionStmt *method)
{
    ...
    {
        argIt->setName(method->parameters[idx].name.lexeme);
    }
