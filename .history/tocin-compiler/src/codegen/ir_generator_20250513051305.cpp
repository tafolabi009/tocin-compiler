...
// AST Visitor method implementations
...
void IRGenerator::visitVariableStmt(ast::VariableStmt* stmt) {
    ...
    llvm::AllocaInst* alloca = createEntryBlockAlloca(currentFunction, stmt->name.lexeme, type);
    if (!alloca) return;
    
    // Store the initial value if one was provided
    if (initValue) {
        builder.CreateStore(initValue, alloca);
    } else {
        // Initialize with a default value based on type
        llvm::Value* defaultValue = createDefaultValue(type);
        if (defaultValue) {
    ...
    currentScope->define(stmt->name.lexeme, alloca);
}

// Many additional visitor methods would be implemented here...

// Helper for creating default values for various types
llvm::Value* IRGenerator::createDefaultValue(llvm::Type* type) {
    if (type->isIntegerTy()) {
        ...
        // For structs, would need to initialize all members
...
bool PatternVisitor::visitPattern(ast::PatternPtr pattern, llvm::BasicBlock* successBlock, llvm::BasicBlock* failBlock) {
    if (auto wildcard = std::dynamic_pointer_cast<ast::WildcardPattern>(pattern)) {
        return visitWildcardPattern(wildcard.get(), successBlock, failBlock);
    } else if (auto literal = std::dynamic_pointer_cast<ast::LiteralPattern>(pattern)) {
        return visitLiteralPattern(literal.get(), successBlock, failBlock);
    } else if (auto variable = std::dynamic_pointer_cast<ast::VariablePattern>(pattern)) {
        return visitVariablePattern(variable.get(), successBlock, failBlock);
    } else if (auto constructor = std::dynamic_pointer_cast<ast::ConstructorPattern>(pattern)) {
        return visitConstructorPattern(constructor.get(), successBlock, failBlock);
    } else if (auto tuple = std::dynamic_pointer_cast<ast::TuplePattern>(pattern)) {
        return visitTuplePattern(tuple.get(), successBlock, failBlock);
    } else if (auto structPattern = std::dynamic_pointer_cast<ast::StructPattern>(pattern)) {
        return visitStructPattern(structPattern.get(), successBlock, failBlock);
    } else if (auto orPattern = std::dynamic_pointer_cast<ast::OrPattern>(pattern)) {
        return visitOrPattern(orPattern.get(), successBlock, failBlock);
    } else {
...
bool PatternVisitor::visitWildcardPattern(ast::WildcardPattern* pattern, 
    ...
    generator.builder.CreateBr(successBlock);
    bindingSuccess = true;
    return true;
}
