...
void IRGenerator::visitCallExpr(ast::CallExpr* expr) {
    ...
    if (expr->callee->name == "print") {
        ...
        llvm::FunctionCallee printFunc = module->getFunction("print");
        if (!printFunc.getCallee()) {
            throw std::runtime_error("Undefined function: print");
        }
...
{
    throw std::runtime_error("Variable statements not implemented in IRGenerator");
}
