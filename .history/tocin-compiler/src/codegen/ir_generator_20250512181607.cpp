...
void IRGenerator::visitFunctionStmt(ast::FunctionStmt *stmt)
{
    ...
    if (stmt->isGeneric()) {
        // For generic functions, we don't generate code immediately
        // Instead, we instantiate them when needed
        
        // Just register it in a table for later instantiation
        // This would be handled by the call expression visitor
        
        // For now, we can generate a placeholder function that reports an error
        // when called without proper type arguments
        ...
        // Add an error message in the function body
        llvm::BasicBlock* block = llvm::BasicBlock::Create(context, "entry", function);
        builder.SetInsertPoint(block);
        
        // Call a runtime error function
        llvm::Function* errorFunc = getStdLibFunction("__generic_function_error");
        if (errorFunc) {
            builder.CreateCall(errorFunc, {
                builder.CreateGlobalStringPtr(stmt->name, "generic_func_name")
            });
        }
        
        // Return a default value if not void
        if (returnType->isVoidTy()) {
            builder.CreateRetVoid();
        } else {
            builder.CreateRet(createDefaultValue(returnType));
        }
        
        return;
    }
    
    // The rest of the function implementation for non-generic functions
    // ... existing non-generic function implementation ...
}
                                                     ...
                                                     const std::vector<ast::TypePtr>& typeArgs)
{
    ...
    // If we don't have a terminator (e.g., return), add one    if (!builder.GetInsertBlock()->getTerminator()) {
