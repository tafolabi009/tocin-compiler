...
namespace type_checker
{
    ...
    void TypeChecker::visitNewExpr(ast::NewExpr *expr)
    {
        ...
        if (expr->getSizeExpr())
        {
            ...
            if (sizeType->toString() != "int")
            {
                errorHandler_.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
                                          "Array size must be an integer, got " + sizeType->toString(),
            ...
            currentType_ = std::make_shared<ast::GenericType>(expr->getKeyword(), "Array", {typeExpr});
        }
        else
        {
