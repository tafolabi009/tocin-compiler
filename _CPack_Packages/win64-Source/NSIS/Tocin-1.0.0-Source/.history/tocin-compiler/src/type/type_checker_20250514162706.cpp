...
namespace type_checker
{
    ...
    TypeChecker::TypeChecker(error::ErrorHandler &errorHandler,
        ...
        : errorHandler_(errorHandler), compilationContext_(compilationContext),
          featureManager_(featureManager),
          environment_(std::make_shared<Environment>()), globalEnv_(environment_) {}
    ...
    {
        // Check each expression in the interpolation
        for (const auto &subExpr : expr->getExpressions())
        {
            subExpr->accept(*this);
            // For now, we don't enforce type restrictions on interpolated expressions
            // They will be converted to strings during code generation
        }

        // String interpolation expressions always result in a string
        currentType_ = std::make_shared<ast::SimpleType>(
            lexer::Token(lexer::TokenType::STRING, "string", "", 0, 0));
    }
    ...
    // Add implementations for ArrayLiteralExpr and cross-namespace visitor methods
    void type_checker::TypeChecker::visitArrayLiteralExpr(ast::ArrayLiteralExpr *expr)
    {
            ...
            else if (!isAssignable(currentType_, elementType))
            {
                ...
                else
                {
                    errorHandler_.reportError(error::ErrorCode::T001_TYPE_MISMATCH,
                                              ...
                                              expr->token.file, expr->token.line, expr->token.column);
                    break;
                }
            }
        }
    ...
    void type_checker::TypeChecker::visitMoveExpr(void *expr)
    {
    ...
    void type_checker::TypeChecker::visitGoExpr(void *expr)
    {
    ...
    void type_checker::TypeChecker::visitRuntimeChannelSendExpr(void *expr)
    {
        // This will be properly implemented when concurrency is fully integrated
        currentType_ = std::make_shared<ast::SimpleType>(
            lexer::Token(lexer::TokenType::IDENTIFIER, "void", "", 0, 0));
    ...
    void type_checker::TypeChecker::visitRuntimeChannelReceiveExpr(void *expr)
    {
        // This will be properly implemented when concurrency is fully integrated
        currentType_ = std::make_shared<ast::SimpleType>(
            lexer::Token(lexer::TokenType::IDENTIFIER, "any", "", 0, 0));
    ...
    void type_checker::TypeChecker::visitRuntimeSelectStmt(void *stmt)
    {
        // This will be properly implemented when concurrency is fully integrated
        currentType_ = std::make_shared<ast::SimpleType>(
            lexer::Token(lexer::TokenType::IDENTIFIER, "void", "", 0, 0));
    ...
    void type_checker::TypeChecker::visitChannelSendExpr(ast::ChannelSendExpr *expr) {
        // Placeholder implementation for channel send expression
        currentType_ = nullptr;
        errorHandler_.reportWarning(
    ...
    void type_checker::TypeChecker::visitChannelReceiveExpr(ast::ChannelReceiveExpr *expr) {
    ...
    void type_checker::TypeChecker::visitSelectStmt(ast::SelectStmt *stmt) {
