...
namespace ir_generator
{
    ...
    {
        expr->left->accept(*this);
        llvm::Value *left = lastValue; // Changed from currentValue to lastValue
        if (!left)
            return;

        expr->right->accept(*this);
        llvm::Value *right = lastValue; // Changed from currentValue to lastValue
        if (!right)
            return;
        ...
        case lexer::TokenType::PLUS:
            if (isInt)
            {
                lastValue = builder.CreateAdd(left, right, "addtmp"); // Changed from currentValue to lastValue
            }
            else if (isFloat)
            {
                lastValue = builder.CreateFAdd(left, right, "addtmp"); // Changed from currentValue to lastValue
            }
            else if (isPointer && right->getType()->isIntegerTy())
            {
                // Pointer arithmetic
                lastValue = builder.CreateGEP(left, right, "ptradd"); // Changed from currentValue to lastValue
            }
            else
            {
                errorHandler.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
        ...
        case lexer::TokenType::MINUS:
            if (isInt)
            {
                lastValue = builder.CreateSub(left, right, "subtmp"); // Changed from currentValue to lastValue
            }
            else if (isFloat)
            {
                lastValue = builder.CreateFSub(left, right, "subtmp"); // Changed from currentValue to lastValue
            }
            else
            {
                errorHandler.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
        ...
        case lexer::TokenType::STAR:
            if (isInt)
            {
                lastValue = builder.CreateMul(left, right, "multmp"); // Changed from currentValue to lastValue
            }
            else if (isFloat)
            {
                lastValue = builder.CreateFMul(left, right, "multmp"); // Changed from currentValue to lastValue
            }
            else
            {
                errorHandler.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
        ...
        case lexer::TokenType::SLASH:
            if (isInt)
            {
                lastValue = builder.CreateSDiv(left, right, "divtmp"); // Changed from currentValue to lastValue
            }
            else if (isFloat)
            {
                lastValue = builder.CreateFDiv(left, right, "divtmp"); // Changed from currentValue to lastValue
            }
            else
            {
                errorHandler.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
        ...
        case lexer::TokenType::PERCENT:
            if (isInt)
            {
                lastValue = builder.CreateSRem(left, right, "modtmp"); // Changed from currentValue to lastValue
            }
            else
            {
                errorHandler.reportError(error::ErrorCode::T006_INVALID_OPERATOR_FOR_TYPE,
        ...
        case lexer::TokenType::EQUAL_EQUAL:
            if (isInt || left->getType()->isPointerTy())
            {
                lastValue = builder.CreateICmpEQ(left, right, "eqtmp"); // Changed from currentValue to lastValue
            }
            else if (isFloat)
            {
