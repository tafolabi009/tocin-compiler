...
namespace ir_generator
{
    ...
    void IRGenerator::visitExpressionStmt(ast::ExpressionStmt *stmt)
    {
        ...
        else if (auto binaryExpr = std::dynamic_pointer_cast<ast::BinaryExpr>(stmt->expression))
        {
            ...
            case lexer::TokenType::PERCENT:
                // These operators have side effects, so we need to store the result
                break;
            default:
                // These operators do not have side effects, so we can discard the result
                lastValue = nullptr;
                break;
            }
        }
        ...
        {
            // If the expression is not a unary or binary expression, we can discard the result
            lastValue = nullptr;
        }
    }
    ...
    void IRGenerator::visitNewExpr(ast::NewExpr *expr)
    {
        ...
        {
            // If type is already a pointer, get the element type
            llvmType = llvm::cast<llvm::PointerType>(llvmType)->getElementType();
        }
        ...
        llvm::Value *typedPtr = builder.CreateBitCast(rawPtr, llvm::PointerType::getUnqual(llvmType), "typedmem");
        lastValue = typedPtr;
    }
    ...
    void IRGenerator::visitDeleteExpr(ast::DeleteExpr *expr)
    {
        ...
        // Cast pointer to i8*
        llvm::Value *voidPtr = builder.CreateBitCast(ptr, llvm::PointerType::get(llvm::Type::getInt8Ty(context), 0), "voidptr");

        // Declare or get free
        llvm::Function *freeFunc = module->getFunction("free");
        if (!freeFunc)
        {
            std::vector<llvm::Type *> params = {llvm::PointerType::get(llvm::Type::getInt8Ty(context), 0)};
