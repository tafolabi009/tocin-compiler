...
llvm::Value *IRGenerator::visitArrayLiteralExpr(ast::ArrayLiteralExpr *expr)
{
    if (expr->elements.empty())
    {
        // Create empty array
        llvm::Type *elementType = getLLVMType(expr->type->elementType);
        llvm::Type *arrayType = createOpaquePtr(elementType);
        llvm::Value *size = llvm::ConstantInt::get(llvm::Type::getInt64Ty(context), 0);
        lastValue = createEmptyArray(arrayType, size);
        return;
    }
    // ... rest of the function ...
}
