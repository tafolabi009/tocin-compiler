...
namespace codegen {
    ...
    llvm::Type* IRGenerator::getLLVMType(std::shared_ptr<Tocin::Type> tocinType) {
        ...
        if (tocinType->toString() == "int") { // Assuming Type has a toString() or similar identifier
    ...
    void IRGenerator::visitBlockStmt(BlockStmt* stmt) {
        // Enter new scope (e.g., push a new map onto a stack of namedValues)
        // std::map<std::string, llvm::AllocaInst*> oldNamedValues = namedValues; // Simple backup

        for (const auto& statement : stmt->statements) {
            statement->accept(this);
             if (errorHandler.hasFatalErrors()) return; // Stop if error occurred
        }

        // Exit scope (e.g., pop the map, restore old namedValues)
        // namedValues = oldNamedValues; // Restore previous scope
    }
    ...
    void IRGenerator::visitExpressionStmt(ExpressionStmt* stmt) {
        ...
        // The result is stored in 'lastValue' but ignored here unless needed for side effects.
         if (errorHandler.hasFatalErrors()) return;
    }
    ...
    void IRGenerator::visitLiteralExpr(LiteralExpr* expr) {
        switch (expr->value.type) {
         ...
         if (errorHandler.hasFatalErrors()) lastValue = nullptr; // Ensure lastValue is null on error
    }
    ...
    void IRGenerator::visitVariableExpr(VariableExpr* expr) {
        ...
        lastValue = builder.CreateLoad(storedType, alloca, expr->name.lexeme.c_str());
         if (errorHandler.hasFatalErrors()) lastValue = nullptr;
    }
    ...
    void IRGenerator::visitBinaryExpr(BinaryExpr* expr) {
         errorHandler.reportError(error::ErrorCode::C001_UNIMPLEMENTED_FEATURE,
                                 "visitBinaryExpr not implemented.", expr->op.line, expr->op.column,
                                 error::ErrorSeverity::ERROR);
         // Needs to:
         // 1. Visit left operand, get valueL.
         // 2. Visit right operand, get valueR.
         // 3. Check types of valueL and valueR. They should generally match or be convertible.
         // 4. Based on operator type (+, -, *, /, <, <=, >, >=, ==, !=):
         //    - Generate appropriate LLVM instruction (e.g., CreateAdd, CreateSub, CreateMul, CreateSDiv, CreateUDiv, CreateFAdd, CreateFSub, CreateFMul, CreateFDiv).
         //    - Generate appropriate comparison instruction (e.g., CreateICmpEQ, CreateICmpNE, CreateICmpSLT, etc. for integers; CreateFCmpOEQ, CreateFCmpONE, etc. for floats).
         lastValue = nullptr;
    }
