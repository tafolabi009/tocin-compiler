 ...
 */
class LinqExpr : public ast::Expr
{
public:
    ...
    LinqExpr(ast::ExprPtr source, LinqOperation operation, ast::ExprPtr predicate = nullptr)
        : source(source), operation(operation), predicate(predicate) {}
    ...
    bool validatePredicate(LinqOperation operation, ast::TypePtr collectionType, ast::TypePtr predicateType)
    {
        ...
        if (!elementType)
        {
            errorHandler.reportError(
                ...
                "Cannot determine element type of collection",
        ...
        switch (operation)
        {
            ...
            case LinqOperation::SELECT:
                ...
                return std::make_shared<ast::GenericType>(
                    "list", 
                    std::vector<ast::TypePtr>{std::make_shared<ast::BasicType>(ast::TypeKind::VOID)});
