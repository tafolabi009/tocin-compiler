...
namespace codegen
{
    ...
    public:
        ...
        // New methods for opaque pointer handling
        /**
         * @brief Register a value and its associated type in the type registry
         * 
         * With LLVM opaque pointers, we need to track the types of values manually
         * since the type information is no longer stored in the pointer itself.
         * 
         * @param value The LLVM value (typically a pointer)
         * @param type The type that the value points to
         */
        void registerType(llvm::Value* value, llvm::Type* type);
        
        /**
         * @brief Get the type associated with a value from the type registry
         * 
         * @param value The LLVM value to look up
         * @return llvm::Type* The associated type, or nullptr if not found
         */
        llvm::Type* getRegisteredType(llvm::Value* value);
        
        /**
         * @brief Create a load instruction with proper type handling for opaque pointers
         * 
         * @param ptr The pointer to load from
         * @return llvm::Value* The loaded value
         */
        llvm::Value* createLoad(llvm::Value* ptr);
        
        /**
         * @brief Create a store instruction with proper type handling for opaque pointers
         * 
         * @param value The value to store
         * @param ptr The pointer to store to
         * @return llvm::Value* The store instruction
         */
        llvm::Value* createStore(llvm::Value* value, llvm::Value* ptr);
        
        /**
         * @brief Create a GEP instruction with proper type handling for opaque pointers
         * 
         * @param ptr The pointer to index into
         * @param indices The indices for the GEP
         * @param elementType The type of the element being indexed
         * @param name Optional name for the result
         * @return llvm::Value* The GEP instruction
         */
        llvm::Value* createGEP(llvm::Value* ptr, std::vector<llvm::Value*> indices, 
                              llvm::Type* elementType, const std::string& name = "");
        
        /**
         * @brief Handle pointer dereferencing with opaque pointers
         * 
         * @param pointerValue The pointer value to dereference
         * @return llvm::Value* The dereferenced value
         */
        llvm::Value* visitPointerDereference(llvm::Value* pointerValue);
        
        /**
         * @brief Create a GEP instruction for array indexing with opaque pointers
         * 
         * @param arrayPtr The pointer to the array
         * @param index The index value
         * @param elementType The type of the array elements
         * @return llvm::Value* The pointer to the indexed element
         */
        llvm::Value* createArrayIndexGEP(llvm::Value* arrayPtr, llvm::Value* index, llvm::Type* elementType = nullptr);
        
        /**
         * @brief Generate instructions for malloc with opaque pointers
         * 
         * @param allocType The type to allocate
         * @param numElements The number of elements to allocate
         * @return llvm::Value* The allocated pointer
         */
        llvm::Value* createMalloc(llvm::Type* allocType, llvm::Value* numElements);
        
        /**
         * @brief Generate a load from a variable with opaque pointer handling
         * 
         * @param name The name of the variable
         * @return llvm::Value* The loaded value
         */
        llvm::Value* generateLoadFromVariable(const std::string& name);
    ...
    private:
        ...
        // Type registry for opaque pointer handling - maps values to their associated types
        static std::map<llvm::Value*, llvm::Type*> typeRegistry;
