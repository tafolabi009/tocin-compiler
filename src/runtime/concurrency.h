...
namespace runtime
{
        ...
        bool validateGoroutineLaunch(ast::ExprPtr function, const std::vector<ast::ExprPtr> &arguments)
        {
            if (!function)
            {
                errorHandler.reportError(
                    error::ErrorCode::C013_INVALID_SPAWN_OPERATION,
    ...
    public:
        ...
        bool validateChannelSend(ast::ExprPtr channel, ast::ExprPtr value, ast::TypePtr channelType, ast::TypePtr valueType)
        {
            ...
            {
                errorHandler.reportError(
                    error::ErrorCode::C011_INVALID_CHANNEL_OPERATION,
            ...
            if (!isValidChannelType(channelType))
            {
                errorHandler.reportError(
                    error::ErrorCode::C011_INVALID_CHANNEL_OPERATION,
                    "Invalid channel type for send operation",
                    "", 0, 0, error::ErrorSeverity::ERROR);
                return false;
            }

            if (!isValidChannelValue(valueType, channelType))
            {
                errorHandler.reportError(
                    error::ErrorCode::C011_INVALID_CHANNEL_OPERATION,
                    "Value type does not match channel type",
                    "", 0, 0, error::ErrorSeverity::ERROR);
                return false;
            }

            if (!isChannelSendable(channelType))
            {
                errorHandler.reportError(
                    error::ErrorCode::C011_INVALID_CHANNEL_OPERATION,
                    "Channel is not sendable (may be receive-only)",
                    "", 0, 0, error::ErrorSeverity::ERROR);
                return false;
            }
        ...
        bool isValidChannelType(ast::TypePtr channelType)
        {
            // Check if the type is a channel type
            if (auto genericType = std::dynamic_pointer_cast<ast::GenericType>(channelType))
            {
                return genericType->name == "Channel";
            }
            return false;
        }

        bool isValidChannelValue(ast::TypePtr valueType, ast::TypePtr channelType)
        {
            // Check if the value type matches the channel's element type
            if (auto genericType = std::dynamic_pointer_cast<ast::GenericType>(channelType))
            {
                if (genericType->name == "Channel" && !genericType->typeArguments.empty())
                {
                    auto elementType = genericType->typeArguments[0];
                    return valueType->toString() == elementType->toString();
                }
            }
            return false;
        }

        bool isChannelSendable(ast::TypePtr channelType)
        {
            // For now, assume all channels are sendable
            // In a more sophisticated implementation, this would check channel direction
    ...
    public:
        ...
        virtual void accept(ast::Visitor &visitor) override
        {
            visitor.visitRuntimeChannelReceiveExpr(static_cast<void *>(this));
        }
    ...
    public:
        ...
        virtual void accept(ast::Visitor &visitor) override
        {
            visitor.visitRuntimeSelectStmt(static_cast<void *>(this));
        }
    };
