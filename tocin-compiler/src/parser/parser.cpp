...
std::shared_ptr<ast::FunctionStmt> Parser::functionDeclaration() {
    ...
    if (match(TokenType::ARROW)) {
        ...
        // Default to void/None return type
...
std::shared_ptr<ast::Statement> Parser::statement() {
    ...
    if (match(TokenType::IMPORT) || match(TokenType::FROM)) return importStatement();
...
std::shared_ptr<ast::Expression> Parser::call() {
    ...
    while (true) {
        if (match(TokenType::LEFT_PAREN)) {
            ...
            // Array indexing is syntactic sugar for a method call on the list
            Token bracketToken = previous();
            std::shared_ptr<ast::Expression> index = expression();
            consume(TokenType::RIGHT_BRACKET, "Expect ']' after index.");
            
            // Convert list[index] to list.__getitem__(index)
            Token getItemToken(TokenType::IDENTIFIER, "__getitem__", bracketToken.filename, 
...
std::shared_ptr<ast::Expression> Parser::primary() {
    if (match(TokenType::FALSE)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::BOOLEAN, "False");
    }
    
    if (match(TokenType::TRUE)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::BOOLEAN, "True");
    }
    
    if (match(TokenType::NIL)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::NIL, "None");
    }
    
    if (match(TokenType::INTEGER_LITERAL)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::INTEGER, previous().value);
    }
    
    if (match(TokenType::FLOAT_LITERAL)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::FLOAT, previous().value);
    }
    
    if (match(TokenType::STRING_LITERAL)) {
        return std::make_shared<ast::LiteralExpr>(previous(), 
               ast::LiteralExpr::LiteralType::STRING, previous().value);
    }
...
std::shared_ptr<ast::Expression> Parser::lambda() {
    ...
    // Parse lambda body - can be a block or a single expression
